...
public class LispSemanticSequencer extends AbstractDelegatingSemanticSequencer {
	...
	protected void sequence_Define(ISerializationContext context, Define semanticObject) {
		if (errorAcceptor != null) {
			...
			if (transientValues.isValueTransient(semanticObject, LispPackage.Literals.DEFINE__EXPRESSION) == ValueTransient.YES)
				errorAcceptor.accept(diagnosticProvider.createFeatureValueMissing(semanticObject, LispPackage.Literals.DEFINE__EXPRESSION));
		}
		SequenceFeeder feeder = createSequencerFeeder(context, semanticObject);
		feeder.accept(grammarAccess.getDefineAccess().getNameIDTerminalRuleCall_1_0(), semanticObject.getName());
		feeder.accept(grammarAccess.getDefineAccess().getExpressionExpressionParserRuleCall_3_0(), semanticObject.getExpression());
		feeder.finish();
	}
	...
	protected void sequence_Inteiros(ISerializationContext context, Inteiros semanticObject) {
		if (errorAcceptor != null) {
			if (transientValues.isValueTransient(semanticObject, LispPackage.Literals.INTEIROS__VALUE) == ValueTransient.YES)
				errorAcceptor.accept(diagnosticProvider.createFeatureValueMissing(semanticObject, LispPackage.Literals.INTEIROS__VALUE));
		}
		SequenceFeeder feeder = createSequencerFeeder(context, semanticObject);
